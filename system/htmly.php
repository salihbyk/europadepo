...
// Load auto updater functions
if (file_exists('system/includes/auto_updater.php')) {
...
// Europa Depo Update page
get('/admin/europa-update', function () {
    $user = $_SESSION[site_url()]['user'];
    $role = user('role', $user);
    if (login()) {
        config('views.root', 'system/admin/views');
        if ($role === 'admin') {
            render('europa-update', array(
                'title' => generate_title('is_default', 'Europa Depo Güncelleme'),
                'description' => safe_html(strip_tags(blog_description())),
                'canonical' => site_url(),
                'metatags' => generate_meta(null, null),
                'type' => 'is_admin-europa-update',
                'is_admin' => true,
                'bodyclass' => 'admin-europa-update',
                'breadcrumb' => '<a href="' . site_url() . '">' . config('breadcrumb.home') . '</a> &#187; Europa Depo Güncelleme'
            ));
        } else {
            render('denied', array(
                'title' => generate_title('is_default', 'Erişim Reddedildi'),
                'description' => safe_html(strip_tags(blog_description())),
                'canonical' => site_url(),
                'metatags' => generate_meta(null, null),
                'type' => 'is_admin-denied',
                'is_admin' => true,
                'bodyclass' => 'denied',
                'breadcrumb' => '<a href="' . site_url() . '">' . config('breadcrumb.home') . '</a> &#187; Erişim Reddedildi'
            ));
        }
    } else {
        $login = site_url() . 'login';
...
// Europa Depo Update settings handler
post('/admin/europa-update-settings', function () {
    if (!login()) {
        $login = site_url() . 'login';
        header("location: $login");
        return;
    ...
    $user = $_SESSION[site_url()]['user'];
    $role = user('role', $user);
    $proper = is_csrf_proper($_POST['csrf']);

    if ($role === 'admin' && $proper) {
        require_once 'system/includes/updater.php';
        $updater = new GitHubUpdater();

        $newConfig = array(
            'github_settings' => array(
                'owner' => $_POST['github_owner'] ?? '',
                'repo' => $_POST['github_repo'] ?? '',
                'branch' => 'main'
            ),
            'backup_before_update' => isset($_POST['backup_before_update']),
            'auto_update_enabled' => isset($_POST['auto_update_enabled'])
        );

        $updater->updateConfig($newConfig);

        $redirect = site_url() . 'admin/europa-update?settings=saved';
        header("location: $redirect");
    } else {
        $redirect = site_url() . 'admin/europa-update';
        header("location: $redirect");
    }
});

// Dismiss update notification
post('/admin/dismiss-update-notification', function () {
    if (!login()) {
...
get('/:static', function ($static) {
    ...
    if (strpos($static, ".xml") !== false) {
        ...
        if (login()) {
            // Otomatik güncelleme kontrolü yap
            if (function_exists('check_for_updates_admin')) {
                check_for_updates_admin();
            }
